...
OBSERVATION needs explicitly revoked mint/freeze authorities, >=$10k liquidity, activity score >=70, and no known hard wallet/concentration risks. Ownership evidence is either fresh graph/insider measurements plus complete holders/creator balances with pool exclusions, or the explicitly partial fallback below. Mode caps are $750k/$5m/$10m; mode volume minimums are $10k/$20k/$50k; at least 30 buys in 5m (FLASH) or 1h (other modes), with buys >=80% of sells. Peak retention must be >=25% when known; reawakening needs two measured renewal signals. Missing bundles/snipers/funding/fresh-wallet coverage or on-chain unique buyers is permitted only in this explicitly unverified tier. During a holder-provider outage or incomplete graph coverage, a fresh GMGN sample can support a yellow observation if it contains >=60 valid wallets covering >=50% of supply after known pool exclusions, with largest observed wallet <=10% and observed top ten <=35%. The expected holder count must be present and consistent. Missing issuer ownership and unseen insider/funding relationships remain UNKNOWN and are disclosed; this sample never permits verified signals. Known concentrations from any available source still block. Delivery rechecks freshness. Observations are deduplicated, can upgrade to a verified signal, and retain later invalidation warnings.
